...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $filtro = array_filter($numeros, function($numero) use ($limite) {
            return $numero > $limite;
        });

        echo "<h2>Números Maiores que $limite:</h2>";
        if (count($filtro) > 0) {
            foreach ($filtro as $numero) {
                echo $numero . "<br>";
            }
        } else {
            echo "Nenhum número maior que $limite foi encontrado.";
        }
    }
    ?>
</body>
